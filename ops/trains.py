...
def get_optimizer(model, name, **kwargs):
    ...
    elif name in ["ACClip", "acclip"]:
    	optimizer = ACClip(model.parameters(), **kwargs)
    else:
...
def train(model, optimizer,
          ...
          train_args, val_args, gpu, model_save_path,
          writer=None,
          snapshot=-1, root="models_checkpoints", dataset_name=None, uid=None,
          verbose=1):
    torch.save(
        {'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict(),'scheduler_state_dict': train_scheduler.state_dict()}, model_save_path + 'warmup')
    ...
    for epoch in range(warmup_epochs):
        ...
        *train_metrics, = train_warm_epoch(optimizer, model, dataset_train,
    ...
    if warmup_epochs > 0:
        ...
        torch.save({'model_state_dict': model.state_dict()}, model_save_path + "warmUpEpoch")
    for epoch in range(epochs):
        batch_time = time.time()
        *train_metrics, = train_epoch(optimizer, model, dataset_train,
                                      smoothing=smoothing, mixup_function=mixup_function,
                                      max_norm=max_norm,scheduler=train_scheduler,
                                      gpu=gpu)
        train_scheduler.step()
        batch_time = time.time() - batch_time
        if ((epoch + 1) % 10 == 0):
            torch.save({'epoch':epoch+1,'model_state_dict': model.state_dict(),'optimizer_state_dict': optimizer.state_dict(),'scheduler_state_dict': train_scheduler.state_dict()},model_save_path + str(epoch + 1))
        if writer is not None and (epoch + 1) % 1 == 0:
...
def train_epoch(optimizer, model, dataset,
    ...
    batch_bar = tqdm(total=len(dataset), dynamic_ncols=True, leave=False, position=0, desc='Train')
    num_correct = 0
    total_loss = 0
    for step, (xs, ys) in enumerate(dataset):
        ...
        batch_size = ys.size(dim=0)
        optimizer.zero_grad()
        logits = model(xs)
        loss = loss_function(logits, ys)
        total_loss += float(loss)
        num_correct += int((torch.argmax(logits, axis=1) == ys).sum())
        nll_meter.update(loss.item())
        loss_meter.update(loss.item())
        batch_bar.set_postfix(
            acc="{:.04f}%".format(100 * num_correct / ((step + 1) * batch_size)),
...
def train_warm_epoch(optimizer, model, dataset,
                smoothing=0.0, mixup_function=None, max_norm=None, scheduler=None, gpu=True):
    model.train()
    if mixup_function is not None:
        loss_function = SoftTargetCrossEntropy()
