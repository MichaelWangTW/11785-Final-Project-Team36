...
def get_dataset(name, root="./data", download=False, **kwargs):
    ...
    if name in ["cifar", "cifar10", "cifar-10"]:
        ...
        dataset_train, dataset_test = get_imagenet100(root=root, **kwargs) #11785 specific
    elif name in ["imagenet"]:
        dataset_train, dataset_test = get_imagenet(root=root, **kwargs)
    elif name in ["svhn"]:
        print("nmsl")
        dataset_train, dataset_test = get_svhn(root=root, **kwargs)
    elif name in ["flowers102"]:
        dataset_train, dataset_test = get_flowers102(root=root, **kwargs)
    else:
...
def get_imagenet(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225),
    ...
    dataset_train = torchvision.datasets.ImageNet(root=train_dir, split='train', transform=transform_train,download=True)
    dataset_test = torchvision.datasets.ImageNet(root=test_dir, split='val',transform=transform_test,download=True)
    return dataset_train, dataset_test
def get_imagenet100(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225),
                 scale=None, ratio=None,
                 hflip=0.5, vflip=0.0,
                 color_jitter=0.0,
                 auto_augment=None,
...
def get_flowers102(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225),
                 scale=None, ratio=None,
                 hflip=0.5, vflip=0.0,
                 color_jitter=0.0,
                 auto_augment=None,
                 interpolation='random',
                 re_prob=0.0, re_mode='const', re_count=1, re_num_splits=0,
                 root="./data", base_folder='flowers102'):
    transform_train = tff.transforms_imagenet_train(
        img_size=224, mean=mean, std=std,
        scale=scale, ratio=ratio,
        hflip=hflip, vflip=vflip,
    ...
    train_dir = os.path.join(root, base_folder, 'train')
    test_dir = os.path.join(root, base_folder, 'val')
    base = os.path.join(root, base_folder)

    dataset_train = torchvision.datasets.Flowers102(root=base, split='train', transform=transform_train,download=True)
    dataset_test = torchvision.datasets.Flowers102(root=base, split='val',transform=transform_test,download=True)

    return dataset_train, dataset_test

def get_svhn(mean=(0.5071, 0.4867, 0.4408), std=(0.2675, 0.2565, 0.2761),
                 padding=None,
                 scale=None, ratio=None,
                 hflip=0.5, vflip=0.0,
                 color_jitter=0.0,
   ...
   #     transform_trains = (transforms.Compose(tfl), *transform_trains[1:])
    ...
    #    img_size=32, mean=mean, std=std,
    #)
    transform_test = [ttf.ToTensor()]
    #dataset_train = torchvision.datasets.SVHN(root=root, split="train", download=True, transform=transform_train)
    #dataset_test = torchvision.datasets.SVHN(root=root, split="test", download=True, transform=transform_test)
    dataset_train = torchvision.datasets.SVHN(root=root, split="train", download=True, transform=ttf.Compose(transform_train))
    dataset_test = torchvision.datasets.SVHN(root=root, split="test", download=True, transform=ttf.Compose(transform_test))
