...
import models.alternet_deformable as alternet_deformable
import ops.meters as meters


def get_model(name, num_classes=10, stem=False, verbose=True, **block_kwargs):
    #AlterNet
    if name in ["alternetV1_jx","alternetv1"]:
        model = alternet.dnn_50(num_classes = num_classes, stem=stem, name = name, **block_kwargs)
        print("create alternet50 success")
    elif name == "alternet_deformable":
        model = alternet_deformable.dnn_50(num_classes = num_classes, stem=stem, name = name, **block_kwargs)
        print("create deform_alternet50 success")
    # AlexNet

    elif name in ["alexnet_dnn", "alexnet"]:
